...
public final class ThreadLocalHelper {
  ...
  private static Thread[] getThreads() {
    ...
    try {
      ...
      threadCountGuess = rootgroup.activeCount();
      do {
        threadCountGuess *= 2;
   ...
   *
   * @param map ThreadLocalMap to be analyzed and cleaned.
   * @param internalTableField Actual Field pointing to 'table'; This avoids re-calculation on every call to this method.
   * @param removeThreadLocalMethod Method to be invoked for removing an Entry
   *
   ...
   *
   * @param internalTableField Field pointing to 'table' holding the ThreadLocals.
   ...
   */
  private static boolean loadedByOurCLorChild(final Object obj) {
    boolean ret = false;
    try {
      LOGGER.trace("--> loadedByOurCLorChild()");
      ...
      if (obj instanceof Collection<?>) {
        ...
        try {
          ...
          LOGGER.warn("loadedByOurCLorChild: Concurrent modification of " + obj, cme);
        }
      }
      LOGGER.trace("Object was not loaded by our ClassLoader/Applicatioon: " + obj);
      ret = false;
      return ret;
    }
    finally {
      LOGGER.trace("<-- loadedByOurCLorChild(); ret = " + ret);
    }
  }
  ...
  private static String getPrettyClassName(final Object obj) {
    if (obj == null) {
      ...
      String pcn = clazz.getCanonicalName();
