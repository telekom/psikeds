KB-META:
01: id w7LampDemoPurePsPvSemLeakFixed
02: name "w7/kbsr L[A|T][M|P][PH|PY] purely ps/pv based / semantic leak fixed"
04: release "0.1.1"
...
event.var WeSeDbScrSoStWithoutMysqlJavaConnex (
  label "TRUE-IF-MysqlJavaConnector-not-used-as-Db-connector" 
    description "...  in the context of aggregating a WeSeDbScriSoSt"
  context [ *pv> WeSeDbScriSoSt *ps> linkToDataStore ]
  NOT
  fact *pv> MysqlJavaConnector
) 
...
logic.enforcer WeSeScriSoStWithoutMysqlJavaConnex  (
  label "Exclude Mysql Java Connector"
  description "Exclude the use of Mysql Java Connector in the context 
  of aggregating a WebServer Scripting with DB Software Stack"
  nexus *pv> WeSeDbScriSoSt 
  evokes *ev> WeSeDbScrSoStWithoutMysqlJavaConnex
)

logic.enforcer WeSeScriSoStWithoutPostgresJavaConnex  (
    label "Exclude Postgres Java Connector"
  description "Exclude the use of Postgres Java Connector in the context 
    of aggregating a WebServer Scripting with DB Software Stack"
  nexus *pv> WeSeDbScriSoSt   
  evokes *ev> WeSeDbScrSoStWithoutPostgresJavaConnex
)
